...
class PacmanGraphics:
    ...
    def swap_images(self, agent_index, new_state):
        ...
        if new_state.is_pacman:
            image = self.drawPacman(new_state, agent_index)
            self.agent_images[agent_index] = (new_state, image)
        else:
            image = self.drawGhost(new_state, agent_index)
            self.agent_images[agent_index] = (new_state, image)
        refresh()
        ...
        for x_num, x in enumerate(wall_matrix):
            ...
            for y_num, cell in enumerate(x):
                if cell:  # There's a wall here
                    ...
                    w_is_wall = self._is_wall(x_num-1, y_num, wall_matrix)
                    e_is_wall = self._is_wall(x_num+1, y_num, wall_matrix)
                    n_is_wall = self._is_wall(x_num, y_num+1, wall_matrix)
                    s_is_wall = self._is_wall(x_num, y_num-1, wall_matrix)
                    nw_is_wall = self._is_wall(x_num-1, y_num+1, wall_matrix)
                    sw_is_wall = self._is_wall(x_num-1, y_num-1, wall_matrix)
                    ne_is_wall = self._is_wall(x_num+1, y_num+1, wall_matrix)
                    se_is_wall = self._is_wall(x_num+1, y_num-1, wall_matrix)
