...
class MultiagentTreeState(object):
    ...
    def get_score(self):
        ...
        if self.state not in self.problem.evaluation:
            raise Exception(
                'get_score() called on non-terminal state or before maximum depth achieved.')
...
class GradingAgent(Agent):
    def __init__(self, seed, student_agent, optimal_actions, alt_depth_actions, partial_ply_bug_actions):
        ...
        self.actions_consistent_with_optimal = [
            True for i in range(len(optimal_actions[0]))]
        self.actions_consistent_with_alternative_depth = [
            True for i in range(len(alt_depth_actions[0]))]
        self.actions_consistent_with_partial_ply_bug = [
            True for i in range(len(partial_ply_bug_actions[0]))]
...
class PacmanGameTreeTest(test_classes.TestCase):
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        if code == 0:
            ...
            self.add_message('State:%s\nStudent Move:%s\nOptimal Move:%s' % (state, student_move, opt_move))
            return self.test_fail(grades)
...
class EvalAgentTest(test_classes.TestCase):
    ...
    def writeSolution(self, module_dict, file_path):
        handle = open(file_path, 'w')
        handle.write('# This is the solution file for %s.\n' % self.path)
